...
misc_files(){
	...
	while ! $WGET $BASEURL/debian-install/{alacritty.deb,stterm.deb,dwm.deb,elpa.tgz,tmux-plugins.tgz,FiraCodeNerdFont.tgz,cn/dict-cn.tgz,cargo.bin.tgz,Sound-test.mp4,clash,yml/config.yaml,starship}; do
...
EOF
	...
	chmod 755 clash starship && sudo mv clash starship /usr/local/bin
}

auto_login(){
    sudo mkdir -p /etc/systemd/system/getty@tty7.service.d/
    
    cat <<EOF | sudo tee /etc/systemd/system/getty@tty7.service.d/override.conf
[Service]
ExecStart=
...
EOF
	...
	sudo cp "$DOTFILE"/etc/apt-fast.conf /etc
	sudo cp -a "$DOTFILE"/etc/kmscon /etc

	sudo ln -f /usr/bin/st /usr/local/bin/xterm
	...
	[ -f "config.yaml" ] && mv config.yaml $DOTFILE/dot.config/clash/
